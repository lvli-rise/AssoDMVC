...
class ModelEngine():
    ...
    def Learn(self, model, criterion, X_train, y_train, X_test, y_test, quiet_mode=False):
        ...
        for i in range(epoch):
            ...
            with torch.no_grad():
                ...
                print("test_loss:{}".format(cls_loss_test.item()))
                ...
                print()
                precision_metric = Precision(task="multiclass", average='macro',
                                             num_classes=self.configs['label_nums']).to(
                    self.configs['device'])
                recall_metric = Recall(task="multiclass", average='macro', num_classes=self.configs['label_nums']).to(
                    self.configs['device'])
                ...
                if accuracy > self.best_acc:
                    ...
                    self.best_F1 = f1
                # 输出结果
                print("Precision:", precision.item(), "Recall:", recall.item(), "F1:", f1.item())
    ...
    def Step(self, training, model, criterion, X, y, i, batch_idx, optimizer=None):
        ...
        if batch_idx % 10000 == 0:
            ...
            print("train_Acc:{}".format(accuracy))
